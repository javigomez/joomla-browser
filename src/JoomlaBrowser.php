...
class JoomlaBrowser extends WebDriver
{
	...
	public function installJoomla()
	{
		...
		$this->debug('I check that FTP tab is not present in installation...');
		$this->debug('otherwise it means that I have not enough permissions to install Joomla and execution will be stopped');
		...
		$I->click(['xpath' => "//fieldset[@id='jform_site_offline']/label[2]"]);
	 ...
	 *
	 * @param   string  $label   The text in the <label> tag of the field
	 * @param   string  $option  The text in the <option> tag that we want to select
	 *
	 * @return void
	 */
	...
	public function selectOptionInChosen($label, $option)
	{
		...
		// Giving time to chosen to close
		$I->wait(1);
	}
